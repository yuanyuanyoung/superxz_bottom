package com.dh.superxz_bottom.framework.db;
...
import com.dh.superxz_bottom.framework.db.exception.DbException;
import com.dh.superxz_bottom.framework.db.sqlite.CursorUtils;
import com.dh.superxz_bottom.framework.db.sqlite.SqlBuilder;
...
public class AfeiDb {
	...
	private static Map<String,AfeiDb> daoMap = new HashMap<String, AfeiDb>();
	...
	public static AfeiDb create(Context ctx){
		DbConfig dbConfig = new DbConfig();
		dbConfig.setCtx(ctx);
		return getInstance(dbConfig);
	}
	
	public static AfeiDb create(Context ctx,String dbName){
		DbConfig dbConfig = new DbConfig();
		dbConfig.setCtx(ctx);
	...
	public static AfeiDb create(Context ctx,String dbName,boolean isDebugSql){
		DbConfig dbConfig = new DbConfig();
		dbConfig.setCtx(ctx);
		dbConfig.setDbName(dbName);
	...
	public static AfeiDb create(Context ctx,boolean isDebugSql){
		DbConfig dbConfig = new DbConfig();
		dbConfig.setCtx(ctx);
		dbConfig.setDebugSql(isDebugSql);
		return getInstance(dbConfig);
	...
	private synchronized static AfeiDb getInstance(DbConfig dbConfig) {
		AfeiDb afeiDb = daoMap.get(dbConfig.getDbName());
		if(null == afeiDb){
